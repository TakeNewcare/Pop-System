...
## 아쉬운 점 및 개선점
1. 각각의 그래프를 객체화하기
   그래프가 필요할 때 마다 매번 눈 빠지게 속성창 보면서 하나하나 그리는 것 보다 이번 프로젝트를 진행하며 만들어진 그래프를 객체화해보고 가능하면 큰 틀을 가져다 쓰는 방식을 구현하는 숙제가 있습니다.<br><br>
2. 현재 배우고 있는 Open CV 기술을 활용하여 생산품의 오류 검출 기능 추가해보기
   해당 프로젝트에는 초음파 신호를 이용하여 단순히 갯수 체크만 가능한 자동화 공정이지만 객체 검출 등을 활용해 좀 더 발전된 프로젝트를 만들어 보겠습니다.<br>
